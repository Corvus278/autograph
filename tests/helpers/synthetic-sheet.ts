 ...
 *
 * Полосы отсчитаны от верхней границы области и равны полутора шагам — той же
 * высоте, по которой трасса ведёт вертикальную границу, поэтому пропажа
 * приходится ровно на полосы, а не на их половины. Четыре, а не три: под
 * пятном обязана скрыться такая доля высоты, чтобы профиль во всю высоту
 * перестал брать барьер по соседям, — иначе лист-фантом ниже проверял бы не
 * полосовую ступень, а всё ту же первую.
 */
