 ...
 * области. С полосами детектора они не совпадают: тот режет кадр на равные
 * полосы от `y = 0` — здесь тринадцать по 61,54 px, — и пятно накрывает три из
 * них целиком, а в две соседние заходит краем. Важна не разбивка, а доля
 * высоты: под пятном обязана скрыться такая её часть, чтобы профиль во всю
 * высоту перестал брать барьер по соседям, — иначе лист-фантом ниже проверял
 * бы не полосовую ступень, а всё ту же первую.
 */
