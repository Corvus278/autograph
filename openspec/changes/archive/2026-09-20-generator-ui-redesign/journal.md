...
### Ручная приёмка 7.3 — замечания пользователя (F2)

1. Лист «Авто — по рецепту» непонятен → переименовать и дать подсказку (каждой странице свой снимок из набора семьи; меняется при «Перегенерировать»).
2. Листы №3 по умолчанию — пользователь отменил.
3. Чернила «Авто» непонятны → убрать вариант «Авто» совсем; по умолчанию — синяя шариковая (BALLPOINT_BLUE); у свотчей подсказка с названием. Сессия со старым ink 'auto' читается как дефолт. Спеку (сценарии «Авто») поправить через /opsx:update.
4. Баг: выпадающий список почерка (HandwritingPicker, Radix Select item-aligned) при прокрутке отрывается от триггера и наезжает на «Реализм» → position="popper" под триггером, ширина триггера, max-h по доступной высоте, прокрутка внутри.

### F2 дополнение

5. В InkPicker не видно выбранную ручку → явная отметка выбранного свотча (кольцо ≥ 3:1 + галочка) и подпись названия тона рядом («Свой цвет» при custom).

### F2

По ручной приёмке: «Авто — по рецепту» → «Разные листы на страницах» с подсказкой; чернила «Авто» удалены, дефолт — синяя шариковая (DEFAULT_INK_TONE_ID), сессия с ink 'auto' → дефолт без потери остальных полей; выбранный свотч — светлое кольцо + галочка, подпись тона рядом (aria-live), «Свой цвет» при custom; Select почерка и shared Select — position popper под триггером (story HandwritingListFollowsTrigger).
Аудит: ok с первого круга; контраст кольца ≥ 8,97:1 на всех тонах.
Спека расходится (правит пользователь через /opsx:update): «Чернила выбираются из палитры» («Авто» первым, сценарий «Авто меняет чернила»), «Перегенерация запускает новый прогон» (упоминания «Авто»); нужен сценарий «дефолт — синяя шариковая, сохранённое auto → дефолт».
Долг: нет отдельной проверки popper у shared Select; подсказка SheetPinSection не через typograf; resolveInkColor → null для неизвестного тона остаётся запасным путём со случайным цветом.
