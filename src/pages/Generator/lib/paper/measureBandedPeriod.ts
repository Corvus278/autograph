 ...
 *
 * Семь сотых — середина плато на том изображении, которое замер и получает: на
 * вырезке по контуру листа охват не меняется на всём окне от 0,01 до 0,13.
 * Снизу окно держат пустые полосы ловушек — 0,004 от медианы, в пятнадцать раз
 * ниже порога; сверху — слабейшие полосы худшего снимка предмета приёмки: у
 * листа, снятого под углом, верх и низ почти без линий, и они дают 0,134, вдвое
 * выше порога.
 *
 * Доля так мала именно потому, что мерой служит вырезка, а не кадр: она убирает
 * столбцы поверхности вокруг листа, медиана найденных полос растёт на пятую
 * часть, слабая полоса — на двадцатую, и их отношение уезжает вниз. Порог,
 * поставленный по кадру целиком, пришёлся бы вплотную к слабым полосам и резал
 * бы ровно по ним.
 */
const MIN_BAND_CONTRAST_SHARE = 0.07;

/**
 * Наименьшая доля полос кадра, обязанных держать гребёнку. Настоящая
 * разлиновка идёт через весь лист: на вырезке по контуру у всех десяти снимков
 * предмета приёмки и у синтетики с дрейфом, с линиями через одну и с крупным
 * шагом доля равна 1,00. Источник периодичности, занявший часть кадра, выше
 * 0,60 не поднимается: полоса текста даёт 0,25—0,50, лист с линиями в половине
 * кадра — 0,50—0,60.
 *
 * Окно порога одностороннее: доля ловушек держится ровно на 0,60 при любой доле
 * контраста от 0,005 до 0,50, а снимки не опускаются ниже 1,00, — годится любое
 * число из (0,60; 1,00]. Семь десятых взяты у нижнего края окна: они оставляют
 * снимку запас почти в треть полос — на ушедшем из резкости дальнем краю и на
 * поверхности, которую вырезка отрезала не везде, — а ловушка, дотянувшаяся до
 * 0,60, всё равно не проходит.
 *
